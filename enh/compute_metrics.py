...
def _map_mask_back(m_tgt: np.ndarray, n_orig: int, sr_orig: int, sr_tgt: int) -> np.ndarray:
    """Mapea máscara booleana de sr_tgt -> sr_orig sin suavizado."""
    if m_tgt is None:
        return None
    if sr_orig == sr_tgt:
        m = m_tgt
    else:
        # i_orig -> i_tgt por vecino más cercano (floor)
        idx = np.floor(np.arange(n_orig, dtype=np.float64) * (sr_tgt / max(sr_orig,1))).astype(int)
        idx = np.clip(idx, 0, len(m_tgt) - 1)
        m = m_tgt[idx]
    return m.astype(bool, copy=False)


def _vad_mask_wrapped(x, sr, backend="webrtc", vad_resample="48k", frame_ms=30, aggr=2):
    tgt_sr = {"none": sr, "16k": 16000, "48k": 48000}.get(vad_resample, sr)
    x_vad = _resample_linear(x, sr, tgt_sr) if tgt_sr != sr else x
    m_tgt, step = _vad_mask_webrtc(x_vad, tgt_sr, frame_ms=frame_ms, aggr=aggr)
    if m_tgt is None:
        return None, None
    m = _map_mask_back(m_tgt, len(x), sr_orig=sr, sr_tgt=tgt_sr)
